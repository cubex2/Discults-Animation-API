...
public class ModelTest extends AnimationModelBase {
	...
	/*
	 * I recommend that you make i class that holds all this information as it is
	 * repeated alone all models. SEE BOTTOM OF CLASS
	 */
	...
	/*
	 * This is where you would register your animations I recommend that you keep
	 * the super active. Uses if statements set priority by putting the highest
	 * priority one at the top.
	 */
	 ...
	 * AnimationModelRenderer body = new AnimationModelRenderer(this, "body");
	 * 
	 * public AnimationModel(String location) { ValveStudioModel model = new
	 * ValveStudioModel(new ResourceLocation(modid, location + ".qc");
	 * body.addCustomModel(new ModelCustomWrapper(model));
	 * body.setRotationPoint(0.0F, 0.0F, 0.0F); body.rotateAngleX =
	 * (-(float)Math.PI/2F); theModel = model; this.setAnimationIncrement(1.0F); }
	 * 
	 * @Override public void render(Entity var1, float f, float f1, float f2, float
	 * f3, float f4, float f5) { super.render(var1, f, f1, f2, f3, f4, f5);
	 * body.render(f5); }
	 * 
	 * }
	 */
	 ...
	 * 
	 * public MyModel() { super("models/mobs/MOBNAME/MOBNAME"); }
	 * 
	 * @SideOnly(Side.CLIENT)
	 * 
	 * @Override public void doAnimation(Entity entity) { super.doAnimation(entity);
	 * 
	 * ANIMCODE }
	 */
	 ...
	 * 
	 * 
	 * if(CODE TO MAKE ENTITY ATTACK) { if(entityMoving(mob)) {
	 * if(mob.animationSwap) { this.setAnimation("walk_attack"); } } else {
	 * if(mob.animationSwap) { this.setAnimation("idle_attack"); } } } else
	 * if(entityMoving(mob)) { if(mob.animationSwap) { this.setAnimation("walk"); }
	 * }
	 * 
	 * else { if(mob.animationSwap) { this.setAnimation("idle"); } } }
	 */
