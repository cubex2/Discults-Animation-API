...
public class SMDModel {
	...
	public SMDModel(SMDModel model, ValveStudioModel owner) {
		...
		this.faces.addAll(model.faces.stream().map((face) -> {
	...
	private void loadSmdModel(ResourceLocation resloc, SMDModel body) throws GabeNewellException {
		InputStream inputStream = new BufferedInputStream(AnimationAPI.class
				.getResourceAsStream("/assets/" + resloc.getResourceDomain() + "/" + resloc.getResourcePath()));
		...
		try {
			...
			throw new GabeNewellException(
					"an error occurred reading the SMD file \"" + resloc + "\" on line #" + this.lineCount, var9);
		}
	...
	/**
	 * public Material requestMaterial(String materialName) throws
	 * GabeNewellException { if (!this.owner.usesMaterials) { return null; } else {
	 * if (this.materialsByName == null) { this.materialsByName = new
	 * HashMap<String, Material>(); }
	 * 
	 * Material result = (Material)this.materialsByName.get(materialName); if
	 * (result != null) { return result; } else { String materialPath =
	 * this.owner.getMaterialPath(materialName); URL materialURL =
	 * SMDModel.class.getResource(materialPath);
	 * 
	 * try { File materialFile = new File(materialURL.toURI()); result = new
	 * Material(materialFile); this.materialsByName.put(materialName, result);
	 * return result; } catch (Exception var6) { throw new
	 * GabeNewellException(var6); } } } }
	 */
