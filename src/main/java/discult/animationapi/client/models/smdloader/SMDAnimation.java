...
public class SMDAnimation
{
    ...
    public SMDAnimation(SMDAnimation anim, ValveStudioModel owner)
    {
        ...
        this.frames.addAll(anim.frames.stream().map((f) -> { return new Frame(f, this); }).collect(Collectors.toList()));
    ...
    private void loadSmdAnim(ResourceLocation resloc) throws GabeNewellException
    {
        ...
        } catch (IOException var7)
        {
            if (lineCount == -1)
            {
                ...
                throw new GabeNewellException("an error occurred reading the SMD file \"" + resloc + "\" on line #" + lineCount, var7);
            }
        }
    }
